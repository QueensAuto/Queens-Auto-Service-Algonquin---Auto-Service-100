            ...
            <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16">
                <div className="h-full flex items-center justify-between">
                    <a href="/" className="flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-cyan-400 rounded-md">
                        <img src="https://queensautoservices.com/wp-content/uploads/Logo-White.webp" alt="Queens Auto Services Logo" className="h-10 w-auto" />
                    </a>
              ...
              <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-slate-900/50 border border-slate-700/50 rounded-lg p-6 text-center">
                    ...
                    <p className="text-slate-400 mt-2 text-sm">{t('fullAddress')}</p>
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-12 max-w-3xl mx-auto rounded-2xl overflow-hidden shadow-2xl shadow-black/50 border-2 border-slate-800">
                <div className={language === 'en' ? '' : 'hidden'}>
                    <div id="wistia_a6i5ic59jv" className="wistia_embed" style={{height: '360px', position: 'relative', width: '640px'}}>&nbsp;</div>
                </div>
                <div className={language === 'es' ? '' : 'hidden'}>
                    <div id="wistia_u9od4mapw5" className="wistia_embed" style={{height: '360px', position: 'relative', width: '640px'}}>&nbsp;</div>
                </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
            <div className="grid md:grid-cols-2 items-center gap-8 md:gap-12">
                ...
                <div className="order-1 md:order-2 relative">
                    <img src="https://queensautoservices.com/wp-content/uploads/Queens-Auto-Services-Algonquin-Front-View.webp" alt={t('aboutImageAlt')} className="rounded-2xl shadow-2xl w-full h-full object-cover" />
...
const ServiceAreaSection: FC<{ t: TFunction }> = ({ t }) => (
    <section className="py-24 px-4">
        <div className="max-w-5xl mx-auto text-center">
            ...
            <div className="mt-12 rounded-2xl overflow-hidden border-4 border-slate-800 shadow-2xl"><img src="https://queensautoservices.com/wp-content/uploads/Queens-Auto-Services-Map.webp" alt={t('mapImageAlt')} className="w-full"/></div>
...
const Footer: FC<{ t: TFunction }> = ({ t }) => (
    <footer className="mt-20 py-10 border-t border-slate-800">
        <div className="max-w-7xl mx-auto px-6 text-center text-sm text-slate-400">
            ...
            <p className="mt-2">{t('fullAddress')}</p>
        </div>
    </footer>
);
            ...
            <div className="relative max-w-md w-full bg-slate-900/60 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-2xl shadow-black/50 text-center transition-all duration-300 scale-100 opacity-100" onClick={e => e.stopPropagation()}>
                ...
                <ul className="space-y-3 mb-6 text-slate-300 text-left max-w-md mx-auto">
                    {['popupBonus1', 'popupBonus2', 'popupBonus3', 'popupBonus4', 'popupBonus5'].map(key => (
                         <li key={key} className="flex items-start gap-3"><span className="text-cyan-400 text-xl flex-shrink-0">✅</span><span dangerouslySetInnerHTML={{__html: t(key)}}></span></li>
                    ))}
                </ul>
...
const TimeSlots: FC<{ t: TFunction, selectedDate: string, selectedTime: string, onTimeSelect: (time: string) => void }> = ({ t, selectedDate, selectedTime, onTimeSelect }) => {
    ...
    const dayOfWeek = selected.getDay(); // 0 for Sunday, 6 for Saturday
    
    const isToday = selected.toDateString() === now.toDateString();

    let endHour = 17; // Default for Saturday (loops up to 16:xx)
    
    if (dayOfWeek >= 1 && dayOfWeek <= 5) { // Weekday
        endHour = 19; // Mon-Fri (loops up to 18:xx)
    }

    for (let hour = 8; hour < endHour; hour++) {
        for (const minute of [0, 30]) {
             if (dayOfWeek >= 1 && dayOfWeek <= 5) { // Weekday
                 if (hour === 18 && minute > 0) continue; // last appt at 6:00pm
             } else if (dayOfWeek === 6) { // Saturday
                 if (hour === 16 && minute > 0) continue; // last appt at 4:00pm
             }
...
const BookingForm: FC<{t: TFunction}> = ({ t }) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const response = await fetch('https://n8n.queensautoservices.com/webhook-test/465c85ff-f19e-4d8b-8907-c806cc9fc07b', {
                method: 'POST',
